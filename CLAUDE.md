...
### Networking
All HTTP traffic goes through `DioClient` (`core/network/dio_client.dart`), exposed as `dioClientProvider`. It sets `baseUrl = ApiConstants.baseUrl` (`https://footrdc.com/wp-json`), 30s timeouts, a debug-only `LogInterceptor`, and a `_RetryInterceptor` that retries connection/timeout errors twice with a 1s delay. Errors are caught and converted via `NetworkExceptionHandler.handle` into the sealed `NetworkException` types (`NoInternetException`, `TimeoutException`, `ServerException`, `ClientException`, `NotFoundException`, `UnauthorizedException`, `RequestCancelledException`, `UnknownException`) with French messages — surface those messages directly in UI. `core/network/http_client_provider.dart` is a deprecated re-export shim; new code should import `dio_client.dart` directly.
